...
export default function DatasetClient({ slug }: { slug: string }) {
	...
	// ---- Initial country (URL → default → first → USA)
	...
	// ---- Series (World Bank only for now)
	...
	// ---- Pull viz config (with sensible fallbacks)
	const viz = product.viz ?? {};
	const chartProps = {
		metricName: viz.metricName ?? product.name,
		unitLabel: viz.unitLabel ?? data?.unit ?? "Value",
		yPrefix: viz.yPrefix,
		ySuffix: viz.ySuffix,
	...
	return (
		<main className="mx-auto max-w-3xl p-6 space-y-6">
			...
			{/* Description (static for now; can move into CATALOG later) */}
			...
			<section className="flex flex-wrap gap-2 text-sm">
				{data?.source?.url ? (
					<a href={data.source.url} target="_blank" rel="noreferrer" className="no-underline">
						{/* Neutral chip + animated sweep on hover */}
						<span className="chip btn-sweep">Source: World Bank API</span>
					</a>
				) : (
					<span className="chip">Source: World Bank</span>
				)}
				<span className="chip">Frequency: Annual</span>
				{data?.unit ? <span className="chip">Unit: {data.unit}</span> : null}
			</section>
		</main>
	);
}
