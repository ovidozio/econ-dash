...
/* ---------- helpers: parse time + default formatters ---------- */

function parseTimeToNumber(t: string): number {
	if (/^\d{4}$/.test(t)) return Number(t); // YYYY
	if (/^\d{4}-\d{2}$/.test(t)) {
		// YYYY-MM
		return Date.UTC(Number(t.slice(0, 4)), Number(t.slice(5, 7)) - 1, 1);
	}
	const q = t.match(/^(\d{4})Q([1-4])$/); // YYYYQn
	if (q) return Date.UTC(Number(q[1]), (Number(q[2]) - 1) * 3, 1);
	const d = Date.parse(t); // ISO fallback
...
type Props = {
	...
	metricName?: string;    // tooltip label (default "Value")
	unitLabel?: string;     // Y axis label override
	yPrefix?: string;       // "$"
	ySuffix?: string;
...
export default function SeriesChart({
	...
	// Disable text selection globally while dragging (prevents page highlight)
	useEffect(() => {
		if (!dragging) return;
	...
	// finalize selection if mouse is released (inside or outside chart)
	useEffect(() => {
		const onUp = () => {
	...
	const onDown = (e: any) => {
		// Prevent default to avoid selecting text on quick drags
	...
	// --- custom tooltip: Year + metric with themed colors
	...
	return (
		<div className="space-y-3">
			...
			<div
				...
				style={{ height }}
				onDoubleClick={resetZoom}        // fallback: dblclick outer container also resets
				onMouseDown={(e) => e.preventDefault()} // extra guard against text selection
			>
				<ResponsiveContainer width="100%" height="100%">
					...
					>
						...
						{/* fine, horizontal-only grid (no vertical lines) */}
						...
						{/* Dark-mode friendly tooltip with Year + metric */}
						<Tooltip
