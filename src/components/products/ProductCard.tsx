...
export default function ProductCard({ item }: { item: ProductMeta }) {
	// Treat SVGs as logos by default (contain + white bg).
	const isLogo =
		item.imageFit === "contain" ||
		(item.image && item.image.toLowerCase().endsWith(".svg"));

	// Per-card heights (uniform grid)
	const imgHeights = "h-36 sm:h-40 lg:h-44";

	// Optional scale for logos: 1 = full size; <1 shrinks
	const scale = isLogo ? item.logoScale ?? 1 : 1;

	return (
		<Link href={`/datasets/${item.slug}`} className="block" aria-label={item.name}>
            <Card className="card-sheen-fill h-[380px] sm:h-[400px] lg:h-[420px] flex flex-col overflow-hidden hover:shadow-md transition">
				{/* Image band – FLUSH with card top, only a bottom divider */}
				<div
					className={[
						"relative w-full",
						imgHeights,
						"overflow-hidden", // rely on the card's rounding
                        "-mt-px",
				...
				>
					{item.image ? (
						<div
							...
							style={isLogo ? { transform: `scale(${scale})`, transformOrigin: "center" } : undefined}
						>
							<Image
								src={item.image}
								alt={item.name}
					...
					{/* For photos only: keep legibility fade at bottom */}
					{!isLogo && (
